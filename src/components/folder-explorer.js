/**
 * Folder Explorer Utilities
 *
 * Pure utility module for file/folder operations.
 * State is managed by side-list-state.js - this module only handles I/O and caching.
 */
...
// ========================================
// Cache (stateless - just for performance)
// ========================================

const cache = new Map();  // Cache of directory contents: path -> items[]
let demoMode = false;     // Force dummy data even in Electron
...
const HIDDEN_EXTENSIONS = ['.jxr'];

function filterHiddenFiles(items) {
  return items.filter(item => {
    if (item.isDirectory) return true;
    const ext = item.name.toLowerCase().slice(item.name.lastIndexOf('.'));
    return !HIDDEN_EXTENSIONS.includes(ext);
  });
}
...
export async function loadDirectory(dirPath) {
  ...
  if (cached) {
    return filterHiddenFiles(cached);
  }
  ...
  if (isDummyMode()) {
    ...
    if (dummyContents) {
      const filtered = filterHiddenFiles(dummyContents);
      cache.set(dirPath, filtered);
      return filtered;
    }
    return [];
  }
  ...
  if (result.success) {
    const filtered = filterHiddenFiles(result.items);
    cache.set(dirPath, filtered);
    return filtered;
  } else {
...
export async function pickFolder() {
  ...
  return await window.electronAPI.folderExplorer.pickFolder();
}
...
export function isDummyMode() {
  ...
  return demoMode || !window.electronAPI?.folderExplorer;
}
...
// No-op init for backwards compatibility
export function init() {
  // State is now managed by side-list-state.js
}
